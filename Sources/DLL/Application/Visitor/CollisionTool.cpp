	...
	: _robot(robot)
{
	...
	if (hitboxMidSensorDanger->coinMax.y != hitboxMidSensorDanger->coinMin.y)
	{
		/// Creation des points.
	...
	if (hitboxMidSensorSafe->coinMax.y != hitboxMidSensorSafe->coinMin.y)
	{
		...
		/// Droite et normale
		auto midD1 = p2MidSafe - p1MidSafe;
		auto midD2 = glm::dvec3(-midD1.y, midD1.x, midD1.z);
		/// Enregistrement des segments.
	...
	if (hitboxLeftSensorDanger->coinMax.y != hitboxLeftSensorDanger->coinMin.y)
	{
		/// Creation des points.
	...
	if (hitboxLeftSensorDanger->coinMax.y != hitboxLeftSensorDanger->coinMin.y)
	{
		...
		/// Droite et normale
		auto leftD1 = p2LeftSafe - p1LeftSafe;
		auto leftD2 = glm::dvec3(-leftD1.y, leftD1.x, leftD1.z);
	...
	/// Creation des points.
	if (hitboxRightSensorDanger->coinMax.y != hitboxRightSensorDanger->coinMin.y)
	{
	...
	if (hitboxRightSensorSafe->coinMax.y != hitboxRightSensorSafe->coinMin.y)
	{
		...
		/// Droite et normale
		auto rightD1 = p2RightSafe - p1RightSafe;
		auto rightD2 = glm::dvec3(-rightD1.y, rightD1.x, rightD1.z);
...
void CollisionTool::computeDetection(NoeudCylindre* node)
{
	for (int j = 0; j < 2; j++)
	{		
		if (_robot->getProfile().centerDistanceSensor 
			&& (j == 0 && _robot->getMidSensorDanger()->coinMax.y != _robot->getMidSensorDanger()->coinMin.y
			|| j == 1 && _robot->getMidSensorSafe()->coinMax.y != _robot->getMidSensorSafe()->coinMin.y))
		{
		...
		if (_robot->getProfile().leftDistanceSensor
			&& (j == 0 && _robot->getLeftSensorDanger()->coinMax.y != _robot->getLeftSensorDanger()->coinMin.y
			|| j == 1 && _robot->getLeftSensorSafe()->coinMax.y != _robot->getLeftSensorSafe()->coinMin.y))
		{
		...
		if (_robot->getProfile().rightDistanceSensor
			&& (j == 0 && _robot->getRightSensorDanger()->coinMax.y != _robot->getRightSensorDanger()->coinMin.y
			|| j == 1 && _robot->getRightSensorSafe()->coinMax.y != _robot->getRightSensorSafe()->coinMin.y))
		{
